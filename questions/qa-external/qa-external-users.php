...
*/
	...
	//	Until you edit this function, don't show login, register or logout links
		...
		return array(
			'login' => 'http://daverees4.webfactional.com/dcdev/index.php?r=site/login',
			'logout' => 'http://daverees4.webfactional.com/dcdev/index.php?r=site/logout',
		);
...
*/
	{
     ...
     $userid=(string)Yii::app()->user->dcid;
			mysql_connect("localhost", "daverees4_con", "owzats11") or die(mysql_error());
            mysql_select_db("daverees4_con") or die(mysql_error());
            $result = mysql_query("SELECT email, name FROM User WHERE id='".mysql_real_escape_string($userid)."'") 
or die(mysql_error());  
...
while($row = mysql_fetch_assoc($result)){
		return array(
					'userid' => $userid,
					'publicusername' => $row['name'],
					'email' => $row['email'],
					'level' => ($userid==4) ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
				);
...
/*
	...
	*/	
		...
		if (count($publicusernames)) {
			...
			$results=mysql_query(
				'SELECT username, id FROM users WHERE username IN ('.implode(',', $escapedusernames).')',
				$qa_db_connection
			);
...
*/
	...
	*/	
		...
		if (count($userids)) {
			...
			$results=mysql_query(
				'SELECT name, id FROM User WHERE id IN ('.implode(',', $escapeduserids).')',
				$qa_db_connection
			);
	...
	//	By default, show the public username linked to the Q2A profile page for each user
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]=$usershtml[$userid];
		}
